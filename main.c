...
void g_affichage_plateau(int const tab[][9], int taille_case, int taille_tableau, int phase_de_jeu, int achivement_unlocked, int tableau_achivement[], int sprite_mur, int sp_bord_gauche, int sp_bord_droit,int sp_bord_haut,int sp_bord_bas){
...
int main(void){
  ...
  g_affichage_plateau(tab, taille_case, taille_tableau, phase_de_jeu, 0, achiv_unlocked, sp_mur_normal, sp_bord_gauche, sp_bord_droit, sp_bord_haut, sp_bord_bas); /*On affiche le tableau en fonction de ce qu'il y a dans le tableau */
  ...
  while(1){
	...
	break;
      ...
      /*Partie où l'on vérifie si un pion est bloqué ou non en déterminant le nb de cases libres autour de chaque pion */
	...
	printf("nb_cases_dispo_apres_fn2 %d\n", game_over_joueur2);
      ...
      if (game_over_joueur2 == 0 && game_over_joueur1 == 0){
	phase_de_jeu = PHASE_GAME_OVER_EGALITE;
	printf("EGALITE\n");
	return 1;
      }
      else if(game_over_joueur1 == 0){
	printf("%d\n", phase_de_jeu);
	if (phase_de_jeu == PHASE_DEP_JOUEUR2){ /*On débloque le succès d'avoir battu le joueur 2*/
	  g_affichage_plateau(tab, taille_case, taille_tableau, phase_de_jeu, 1, achiv_unlocked, sp_mur_normal, sp_bord_gauche, sp_bord_droit, sp_bord_haut, sp_bord_bas); /*On affiche le tableau en fonction de ce qu'il y a dans le tableau */
	}
	printf("GAME OVER JOUEUR 1\n");
	return 1;
      }
      else if(game_over_joueur2 == 0){
				
	g_affichage_plateau(tab, taille_case, taille_tableau, phase_de_jeu, 2, achiv_unlocked,
	...
	return 1;
      ...
      if (game_over_joueur2 == 0 || game_over_joueur1 == 0){
	while (!SourisCliquee()); /*Affichage de l'écran de fin ou t'as perdu parce que T'ES MAUVAAAAAIS JACK */
	return 1;
      }
    }
